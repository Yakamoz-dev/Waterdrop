...
/**
 * Class ProfileRemoveItem
 *
 * @package Aheadworks\Sarp2GraphQl\Model\Resolver\Mutation
 */
class ProfileRemoveItem extends AbstractProfileResolver
{
    /**
     * @inheritdoc
     */
    public function performResolve(
        ...
        if (!isset($args['item_id']) || empty($args['item_id'])) {
            throw new GraphQlInputException(__('Specify the "item_id" value.'));
        }

        return $this->profileManagement->removeItem(
            $args['profile_id'],
            $args['item_id']
        );
    }
