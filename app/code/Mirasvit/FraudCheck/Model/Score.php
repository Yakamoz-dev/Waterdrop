...
class Score extends DataObject
{
    ...
    public function getFraudScore($force = false, $save = true)
    {
        if (!$this->hasData('fraud_score') || $force) {
            ...
            foreach ($this->getRules() as $rule) {
                if ($rule->isActive()) {
                    $totalImportance += $rule->getImportance();
                }
            }
            ...
            try {
                foreach ($this->getRules() as $code => $rule) {
                    $tsr = microtime(true);
                    if ($rule->isActive()) {
                        $score += $rule->getFraudScore() * $rule->getImportance() / $totalImportance;
                    }
    ...
    {
        $data = SerializeService::encode($this->_data);
    ...
    public function load()
    {
        ...
        if ($var->getPlainValue()) {
            $this->_data = SerializeService::decode($var->getPlainValue());
        }
