...
//        }
        ...
        if (empty($pro) || $post['line'] == "Offline") {
            $this->messageManager->addError(__('No Corresponding Order No.'));
            $this->_redirect('gift');
            return;
        }
        if ($pro['ro_used']) {
            $this->messageManager->addError(__('Sorry Your Order No was used!'));
            $this->_redirect('gift');
            return;
        }
        if ($pro['ro_regd']) {
            $this->messageManager->addError(__('Sorry Your Order No was regd!'));
            $this->_redirect('gift');
            return;
        }
...
//            $newCustomer->save();
            ...
            $product->setRoRegd(1);
            $product->setPlatform($post['platform']);
            $product->save();
