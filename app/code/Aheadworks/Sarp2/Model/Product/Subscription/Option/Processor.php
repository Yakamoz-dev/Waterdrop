...
class Processor
{
    ...
    private function getFirstPayment($option, $planDefinition, $exclTax, $item = null)
    {
        ...
        if ($isShow) {
            ...
            $amount = $firstPaymentAmount + $option->getInitialFee();
            $fee = $option->getInitialFee();
    ...
    private function getSubscriptionEndsDate($planDefinition)
    {
        ...
        $data = [
            'isShow' => true,
            'type' => self::ENDS,
            'label' => $this->detailsFormatter->getSubscriptionEndsDateLabel(),
            'date' => $stopDate,
            'value' => $stopDate
        ];
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function getOptionPrices($option, $planDefinition, $exclTax = true, $item = null)
    {
        $baseRegularPrice = $this->getBaseRegularPrice($option, $item);
        ...
        if ($this->advancedPricingConfigValueResolver->isUsedAdvancePricing($productId)) {
            ...
            $oldPrice = $this->catalogPriceCalculator->getOldPriceAmount($baseRegularPrice);
            $tierPrices = [];
        }
