...
Productbundle.Option.prototype = {
    ...
    add: function (data) {
        ...
        if (data)
        {
            ...
            $(this.idLabel + '_' + data.index + '_bundle_option').select("option").each(function (node) {
                if (data.bundle_option == $(node).value)
                {
                    $(node).selected = true;
                }
            });
            ...
            if (data.bundle_tags) {
                var bundle_tags = data.bundle_tags.split(",");
                $(this.idLabel + '_' + data.index + '_bundle_tags').select("option").each(function (node) {

                    if (bundle_tags.indexOf($(node).value) != -1)
                    {
                        $(node).selected = true;
                    }
                });
...
jQuery('#bundle_product_save_form').validation();
     if (jQuery('#bundle_product_save_form').validation('isValid')) {
       ...
       jQuery(".product_form_product_form_bundlediscountModal button[data-role='closeBtn']").trigger( "click" );
    }
}
...
jQuery(".bundle_tags").each(function (index) {
